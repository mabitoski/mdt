...
param(
  [string]$ConfigPath,
  [ValidateSet('auto', 'laptop', 'desktop', 'unknown')][string]$Category,
  [string]$ApiUrl,
  [string]$Technician,
  [string]$LogPath,
  [string]$ArtifactRoot,
...
Invoke-FogReportCore -ConfigPath $ConfigPath -FixedParameters @{ TestMode = 'stress' } -ExplicitParameters $explicitParameters
