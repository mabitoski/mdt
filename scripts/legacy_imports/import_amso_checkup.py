...
def parse_segment_one(rows, path, tag, category_map):
    ...
    for idx, row in enumerate(rows[1:], start=2):
        if row_is_blank(row) or looks_like_header(row):
            continue
        serial_raw = resolve_spec(row, positions, "S/N")
        ...
        overall = normalize_status(resolve_spec(row, positions, "OK ?"))
        if overall:
            components["overall"] = overall
        components = components or None
        problems = []
        for spec in ["Problèmes"]:
        ...
        for col in row[6:]:
            if col and str(col).strip():
                extra_notes.append(str(col).strip())
        legacy = {
            "source": "legacy-amso-checkup",
        ...
        if extra_notes:
            legacy["notes"] = extra_notes
        category = guess_category(None, os.path.basename(path))
        if category == "unknown" and serial in category_map:
...
def parse_segment_two(rows, path, tag, category_map):
    ...
    for idx, row in enumerate(rows[1:], start=2):
        ...
        if problems:
            legacy["problems"] = problems
        resolution = resolve_spec(row, positions, "Résolution ?")
        ...
        repair_tech = normalize_technician(resolve_spec(row, positions, ("TECHNICIEN", 2)))
        if repair_tech:
            repair_ok = resolve_spec(row, positions, ("OK ?", 2))
        ...
        if double_tech:
            double_ok = resolve_spec(row, positions, ("OK ?", 3))
            double_components = None
            status = normalize_status(double_ok)
            if status:
                double_components = {"overall": status}
            double_legacy = dict(legacy)
            double_legacy["role"] = "double_check"
            if double_ok:
                double_legacy["ok"] = double_ok
            payloads.append(
                make_payload(
                    serial=serial,
                    mac=mac,
...
def main():
    parser = argparse.ArgumentParser(description="Import PROD - 1er checkup (visuel) (AMSO).csv")
    parser.add_argument(
        "--csv",
        default="/home/christopher/tmp/PROD - 1er checkup (visuel) (AMSO).csv",
    )
    ...
    if not rows:
        ...
        return 0
    split_idx = find_header_row(rows, "DATE_TRAITEMENT")
    ...
    category_map = {}
    if segment_two:
        header = segment_two[0]
        positions = header_positions(header)
        for row in segment_two[1:]:
            if row_is_blank(row) or looks_like_header(row):
                continue
            serial_raw = resolve_spec(row, positions, "NUM_SERIE")
            serial = normalize_serial(serial_raw) if is_serial_candidate(serial_raw) else None
